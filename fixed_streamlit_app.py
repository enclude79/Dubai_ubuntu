...
DB_CONFIG = {
    ...
    'password': os.getenv('DB_PASSWORD', 'EnPswFJWY1wa'),
    'host': os.getenv('DB_HOST', '89.169.166.179'),
...
def get_cheapest_properties_by_area(top_n=3, min_size=None, max_size=None):
    ...
    if min_size is not None:
        query += " AND area >= %s" # area здесь - это площадь из БД
...
def get_property(property_id):
    ...
    query = "SELECT id, title, price, location, property_type, area, rooms, baths, geography, property_url FROM bayut_properties WHERE id = %s"
...
def main():
    ...
    if conn is None:
        ...
        st.info("Проверьте настройки подключения к базе данных PostgreSQL.")
        return
    else:
        try:
            cursor = conn.cursor()
